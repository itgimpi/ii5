...
struct el { // element steka
    ...
    tower start, tmp, finish;
    int n; };

el call(tower start, tower tmp, tower finish, int n) {
    el rez;
    rez.type = 0; // poziv
    rez.start = start; rez.tmp = tmp; rez.finish = finish; rez.n = n;
    return rez;  }

el print(tower start, tower finish) {
    el rez;
    rez.type = 1; // ispis
    rez.start = start; rez.finish = finish;
    return rez;     }

void hanoi(int n, int start, int tmp, int finish) {
    stack<el> s;
    s.push(call(start, tmp, finish, n)); // poziv rek. f-je
